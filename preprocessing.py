...
#	cv2.imshow('image', img_org)
	...
	person_count = 0  # Initialize counter for persons
    
	for i in range(boxes.shape[1]):		
            
	   if scores[0, i] > 0.5 and labels[int(classes[0, i])] == "stop sign":
	       person_count += 1    
	...
	cn=str(person_count)
	cv2.putText(img_org,cn,(10, 25),cv2.FONT_HERSHEY_SIMPLEX,1,(0, 0, 255),2)
